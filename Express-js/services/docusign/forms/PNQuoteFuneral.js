...
class PNQuoteFuneral extends BaseForm {
    ...
    async fetchPNStmtDetails (personId) {
        try {
    ...
    async getAgreementDetails (caseInfoForm) {
        ...
        let pnstmtData = await this.fetchPNStmtDetails(this.person.id)
        pnstmtData = pnstmtData ? (pnstmtData.Agreement ? pnstmtData.Agreement : null) : null
        if (pnstmtData && pnstmtData.id) {
            const agreementDetails = await models.Agreement.findOne({
                where: { id: pnstmtData.id },
                include: [{
            ...
            const spResult = await sequelize.query('GetAgreementDetails :agreementId', {
                replacements: { agreementId: pnstmtData.id },
    ...
    async funeralAssignedToPreFillData () {
        // NOTE: purchaser address and stmt location address fields in docusign template is edited with expanding text box area and size of font
        // So, For prepord/prod, we need to do the same changes in newly copied/original form.
        ...
        const { line1, line2, city, state, country, zipcode } = agreementLocation && agreementLocation.place ? agreementLocation.place.address : ''
        ...
        let contractNumber = agreement ? agreement.contractNumber : ''
        let quotationResult
        if (agreement) {
            quotationResult = await models.Quotation.findOne({
                where: {
                    funeralAgreementId: agreement.id,
                    deletedBy: null,
                    deletedAt: null
                }
            })
            if (quotationResult) {
                contractNumber = quotationResult.quotationNumber
            }
        }
        const data = {
